...
#MAP_MFRI_modelCompare <- data.frame(MAP_mm = rep(seq(400,900,1),each=1000))
#MAP_MFRI_modelCompare$MFRI <- MFRI_from_MAP(MAP = MAP_FF_modelCompare$MAP_mm)

#modelCompare_binom <- ggplot(data = MAP_MFRI_modelCompare, aes(x = MAP_mm, y = 1/MFRI))+
...
# Model ----
  ...
  MAP_seed <- sample(x = seq(from = min(krugerMAP_FRI_df$MAP_mm),
                                to = max(krugerMAP_FRI_df$MAP_mm),
                                by = 1),
                        size = 1)
  MAP_seed  
  MFRI <- MFRI_from_MAP_mean(MAP = MAP_seed,Flat = FireFrequencyFlat)
  MFRI
  ...
  for(timeSkip in 1:timeLimit){
   ...
   Tree_vector_height <- treeGrowth(height_previous = Tree_vector_height, MAP = MAP_seed,Flat = GrowthFlat)
...
dataLoc <- file.path(paste("Runs/",simTime,"_treeHeightsByMAP",".csv",sep = ""))
metadataLoc <- file.path("Runs/",paste(simTime,"_metadata",".csv",sep = ""))
